            ...
            if (!$req->has('bill_id') && !$req->has('id')) {
                ...
                if($req->input('entry_type')=="Payable"){
                    ...
                    $data = array(
                        ...
                        $description = 'Payment to broker ' . $broker->broker_name . 'at ' . $receiving_date . ' ';
                        ...
                        if($receiving_type == 'Cheque'){
                            ...
                            $general_data = array(
                                ...
                                'FKCustomerID' => $broker_id,
                                'FKRecievingID' => $receiving_last_id,
                                'bill_id' => trim($bill_number, '[]"'),
                                'ledger_desc' => $description . ' aginst this cheque#:' . $deposit_slip_number . ' at bank#:' . $get_bank_name[0]->BankName . ' bilty id ' . trim($bill_number, '[]"'),

                            );
                            DB::table('ledger')->insert($general_data);
                            $broker_data = array(
                                'bk_date' => $receiving_date,
                                'bk_desc' => $description . ' aginst this cheque#:' . $deposit_slip_number . ' at bank#:' . $get_bank_name[0]->BankName . ' bilty id' . trim($bill_number, '[]"'),
                                'bk_reference' => $deposit_slip_number,
                                'bk_credit' => $receiving_amount,
                                'FKBrokerID' => $broker_id,
                                'FKRecievingID' => $receiving_last_id,
                                'reference_id' => trim($bill_number, '[]"')
                            );
