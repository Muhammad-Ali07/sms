...
@section('main-content')
    ...
    <div class="flex">
        ...
        <div class="content">
            ...
            @if (empty($select))
                <form method="post" id="yard_form" action="{{ route('yard.store') }}">
                @else
                    <form method="post" id="yardUpdate" action="{{ route('yard.update', ['id' => $select->id]) }}">
            ...
            @endif
                                ...
                                <div id="input" class="p-5">
                                    <div class="preview">
                                        <div class="mt-2 c_name">
                                            <label for="regular-form-2" class="form-label">Yard</label>
                                            <input required id="regular-form-2" type="text"
                                                value="@if (!empty($select)) {{ $select->name }} @endif"
                                                class="form-control " placeholder="Yard name" name="name">
                                        </div>
                ...
                @php 
                    $yards = DB::table('yard')->get();
                @endphp
                @if (!Request::is('*yardEdit*'))
                    <div class="grid">
                   <div class="intro-y flex items-center mt-12">
                      <h2 class="text-lg font-medium mr-auto">
                      </h2>
                               ...
                               <div class="preview">
                                     ...
                                     <tbody>
                                            ...
                                            <tr>
                                                  ...
                                                  <td>{{$item->name}}</td>
                                                  <td>
                                                    <a href="{{route('yard.edit',[$item->id])}}">Edit</a>
                                                    <a class="deleteyard" href="javascript:void(0)" data-url="{{route('yard.delete',[$item->id])}}" data-name="{{$item->name}}">Delete</a>
...
@section('scripts')
    <script>
        ...
        document.querySelectorAll('.deleteyard').forEach(function(element) {
            element.addEventListener('click', function(event) {
                event.preventDefault(); // Prevent default action

                // Get customer name and delete URL from data attributes
                var yardName = this.getAttribute('data-name');
                var deleteUrl = this.getAttribute('data-url');

                // Call the SweetAlert function with dynamic data
                showSweetAlert('warning', 'Are you sure?', "Do you really want to delete " + yardName + "?", deleteUrl);
            });
        });
    </script>
@endsection
