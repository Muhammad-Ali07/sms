...
@section('main-content')
    ...
    <div class="flex">
        ...
        @endphp
            @if (empty($select))
                <form method="post" id="pickup_point_form" action="{{ route('pickupPoint.store') }}">
                @else
                    <form method="post" id="pickup_point_update" action="{{ route('pickupPoint.update', ['id' => $select->id]) }}">
            ...
            @endif
                                ...
                                <div id="input" class="p-5">
                                    <div class="preview">
                                        <div class="mt-2 c_name">
                                            <label for="regular-form-2" class="form-label">Pickup Point</label>
                ...
                @php 
                    $yards = DB::table('pickup_points')->get();
                @endphp
                @if (!Request::is('*pickupPoint-edit*'))
                    <div class="grid">
                   <div class="intro-y flex items-center mt-12">
                      <h2 class="text-lg font-medium mr-auto">
                      </h2>
                                                  ...
                                                  <td>
                                                        @if($user_meta->setup == 1)
                                                            @if($user_meta->edition ==1 )
                                                                <a href="{{route('pickupPoint.edit',[$item->id])}}">Edit</a>
                                                            @endif
                                                            @if($user_meta->deletion == 1)
                                                                <a class="deletePickupPoint" href="javascript:void(0)" data-url="{{route('pickupPoint.delete',[$item->id])}}" data-name="{{$item->name}}">Delete</a>
                                                            @endif    
                                                        @endif
                                                    </td>
                                               </td>
...
@section('scripts')
    <script>
        ...
        document.querySelectorAll('.deletePickupPoint').forEach(function(element) {
