    ...
    <div class="container">
        ...
        <div class="bill-header">
            ...
            <div class="company-name">
                {{$data['report_name']}}
                <br>
                <span class="manager-name">All Customers PNL</span>
            </div>
            <!-- Date Range -->
        ...
        <table class="invoice-details-table">
            ...
            <tbody>
                @php
                    $sr = 1;
                    $groupedBuilties = $data['groupedBuiltiesArray'];
                @endphp
                
                @foreach ($groupedBuilties as $customerName => $builtyRecords)
                    <tr>
                        <th colspan="6">{{$customerName}}</th>
                    </tr>
                    @foreach ($builtyRecords as $b)
                        @php
                            $total_buying = 0;
                            $total_selling = 0;
    
                            $total_selling = DB::table('now_bilty_selling_expense')
                                ->where('builty_id', '=', $b->bid)
                                ->pluck('amount') // Get the amounts as a collection
                                ->map(function($amount) {
                                    // Remove commas and convert to float
                                    return (float) str_replace(',', '', $amount);
                                })
                                ->sum(); // Sum the cleaned amounts
                            $total_buying = DB::table('now_bilty_puchase_expense')
                                ->where('bilty_id', '=', $b->bid)
                                ->pluck('amount') // Get the amounts as a collection
                                ->map(function($amount) {
                                    // Remove commas and convert to float
                                    return (float) str_replace(',', '', $amount);
                                ...
                                ->sum(); // Sum the cleaned amounts
                            $pnl = $total_selling - $total_buying;
