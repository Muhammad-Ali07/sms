...
<style>
  ...
  .buityid  {
         font-size: 41.183px !important;
    }
    
    /* Optional styles for normal screen display */
        #printableArea1234 {
        ...
        @media print {
            ...
            #printableArea1234 {
                padding: 0; /* Removes padding around the printable area */
                margin: 0; /* Removes margins */
                border: none; /* Removes the border if not needed in print */
            }

            /* Optional: Adjust page breaks or remove extra margins/padding */
            @page {
                size: A4; /* Sets the page size to A4 */
                margin: 0; /* Removes margins from the print page */
            }
...
$qrcode = QrCode::size(400)->generate($path);
        ...
        $company = DB::table('selfcompany')->where('id',$sql->self_company)->first();
        $package = DB::table('now_packages')->where('id','=',$sql->package_id)->first();
        
        $package_name = isset($package->packagename) ? $package->packagename : '';
        $typeOfGood = isset($sql->type_of_good) ? $sql->type_of_good : '';
      ...
      foreach($sql1 as $items){
        ...
        // $item[$i] = isset($items->items) ? $items->items : [];
        $brand[$i] = $items->brand;
...
<div id="printableArea1234">
   ...
   <svg version="1.1" id="Group_3_copy" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px"
      y="0px" viewBox="50 -70 2380 3508"
        width="800"   
        height="1100"  
        xml:space="preserve"
      >
       
             <image style="overflow:visible;enable-background:new;" width="2480" height="" id="Background" xlink:href="'.$path.'/A07A285E.png"  transform="matrix(1 0 0 1 0 5.4894)"></image>
      <g>
        ...
        <path id="Shape_3_copy_2" class="st1" d="M1007.1,520.2v182.00"/>
         ...
         <text id="_20_KG" transform="matrix(1 0 0 1 1806.2972 967.9908)" class="st2 st5">{{(!empty($sql->weight)?$sql->weight : '')}}</text>
         ...
         <text id="_01" transform="matrix(1 0 0 1 915.2963 976.9908)" class="st2 st5">{{(!empty($sql->type_of_good)? 1 : '')}}</text>
    ...
    overflow: hidden;
"></foreignObject>
<foreignObject xmlns="http://www.w3.org/2000/svg" id="_1-2" data-name="1" transform="matrix(1 0 0 1 1140.7152 940.525)" class="st6 st7" style="
    position: relative;
    width: 590px;
    display: block;
    font-size: 38px;
    ...
    overflow: hidden;
"></foreignObject>
         <text transform="matrix(1 0 0 1 1140.7152 975.5299)" class="st6 st7">{{!empty($sql->type_of_good) ? $sql->type_of_good : ''}} {{isset($package_name) ? $package_name : ''}}</text>
...
"></foreignObject>
         ...
         <text transform="matrix(1 0 0 1 551.6586 1269.5104)" class="st6 st8">{{(!empty($sql->receivername)? $sql->receivername : '')}}</text>
         
         <text transform="matrix(1 0 0 1 551.6586 1400.3854)" class="st6 st8">{{(!empty($sql->receiverphone)? $sql->receiverphone : '')}}</text>
         ...
         <text transform="matrix(1 0 0 1 553.7094 1606.6569)" class="st6 st8">{{(!empty($sql->generatedby)? $sql->generatedby : '')}}</text>
         <text transform="matrix(1 0 0 1 553.7094 1647.5319)" class="st6 st8"></text>
         <text id="Quantity" transform="matrix(1 0 0 1 893.5853 860.9547)" class="st9 st10">Qty</text>
         ...
         <text id="iSLAMABAD" transform="matrix(1 0 0 1 152.0536 1043.816)" class="st9 st12">Mobile No</text>
         <text id="_aL_yOUSUF_gOODS_" transform="matrix(1 0 0 1 154.9222 1069.3619)" class="st6 st13"></text>
         <text id="gUJRANWALA" transform="matrix(1 0 0 1 152.0536 1161.443)" class="st9 st12">Vehicle No</text>
         ...
         <path id="Shape_6_copy_2" class="st19" d="M2043.3,895.2v765.2"/>
          <path id="Shape_6_copy_3" class="st19" d="M2390.3,450.2v965.2"/>
          <path id="Shape_6_copy_4" class="st19" d="M2390.3,2050.2v965.2"/>
         ...
         <text id="ORIGINAL" transform="matrix(1 0 0 1 2000.00 570.50)" class="buityid st9 st20">Builty# :</text>
         <text id="ORIGINAL_" transform="matrix(1 0 0 1 2000.00 620.50)" class="buityid st9 st20">{{(!empty($sql->builty_id) ? $sql->builty_id : '')}}</text>
         <text id="ORIGINAL" transform="matrix(1 0 0 1 2000.00 720.50)" class="buityid st9 st20">Job No# :</text>
         <text id="ORIGINAL_" transform="matrix(1 0 0 1 2000.00 770.50)" class="buityid st9 st20">{{(!empty($sql->computerno) ? $sql->computerno : '')}}</text>
         ...
         <text transform="matrix(1 0 0 1 520.000 150.0739)" class="st6" style="font-size: 65px;">{{$company->name}}</text>
         <text transform="matrix(1 0 0 1 520.000 220.0739)" class="st6" style="font-size: 55px;">GOODS TRANSPORT COMPANY</text>
         <text transform="matrix(1 0 0 1 520.000 305.0739)" class="st6" style="font-size: 30px">{{$company->address}}</text>
         <text transform="matrix(1 0 0 1 520.000 391.3883)" class="st6"style="font-size: 30px" xml:space="preserve">Phone:{{$company->phone}}</text>
         <text transform="matrix(1 0 0 1 520.000 434.5446)" class="st6" style="font-size: 30px" xml:space="preserve">www.smslogistics.com.pk     smalogistics@yahoo.com</text>
         <text transform="matrix(1 0 0 1 520.000  477.7028)" class="st6" style="font-size: 30px">SNTN: 5215306-4</text>
         <g id="logo">
            <image style="enable-background:new;" width="787" height="440" id="image-2" xlink:href="{{$path2}}/{{$selfcompanyc->logo}}"  transform="matrix(1 0 0 1 1350.4515 70.9059)">
            </image>
         </g>
...
<foreignObject xmlns="http://www.w3.org/2000/svg" id="_1-2" class="st17" d="M814,1403.3h957.2v256.5H814V1403.3z" style="">
         ...
         <text transform="matrix(1 0 0 1 551.6586 3005.5103)" class="st6 st8">{{(!empty($sql->receivername) ? $sql->receivername : '')}}</text>
         <text transform="matrix(1 0 0 1 551.6586 3140.3853)" class="st6 st8">{{(!empty($sql->receiverphone) ? $sql->receiverphone : '')}}</text>
         ...
         <text id="Quantity-2" transform="matrix(1 0 0 1 893.5853 2596.9546)" class="st9 st10">Qty</text>
         ...
         <text id="iSLAMABAD-2" transform="matrix(1 0 0 1 152.0536 2779.8169)" class="st9 st12">Mobile No</text>
         <text id="_aL_yOUSUF_gOODS_2" transform="matrix(1 0 0 1 154.9222 2805.3618)" class="st6 st13"></text>
         <text id="gUJRANWALA-2" transform="matrix(1 0 0 1 152.0536 2897.4438)" class="st9 st12">Vehicle No</text>
        ...
        <path id="Shape_5_copy_4" class="st1" d="M1440.1000,137.7H491.6" transform="matrix(1 0 0 1 332.0536 2970.8707)"></path>
         ...
         <text id="FOR_VEHICLE" transform="matrix(1 0 0 1 2000.00 2310.9517)" class="buityid st9 st20">Builty# :</text>
         <text id="FOR_VEHICLE" transform="matrix(1 0 0 1 2000.00 2360.9517)" class="buityid st9 st20">{{(!empty($sql->builty_id) ? $sql->builty_id : '')}}</text>

         <text id="FOR_VEHICLE" transform="matrix(1 0 0 1 2000.00 2450.9517)" class="buityid st9 st20">job No# :</text>
         <text id="FOR_VEHICLE" transform="matrix(1 0 0 1 2000.00 2510.9517)" class="buityid st9 st20">{{(!empty($sql->computerno) ? $sql->computerno : '')}}</text>
         ...
         <text transform="matrix(1 0 0 1 520.000 1900.0739)" class="st6" style="font-size: 65px;">{{$selfcompanyc->name}}</text>
         <text transform="matrix(1 0 0 1 520.00 2041.0739)" class="st6" style="font-size: 30px">Head Office: {{$selfcompanyc->address}}</text>
         <text transform="matrix(1 0 0 1 520.00 2127.3882)" class="st6" style="font-size: 30px">Phone:{{$selfcompanyc->phone}}</text>
         <text transform="matrix(1 0 0 1 520.00 2170.5444)" class="st6" style="font-size: 30px" xml:space="preserve">www.smslogistics.com.pk  smslogistics@yahoo.com</text>
         <text transform="matrix(1 0 0 1 520.00  2213.7026)" class="st6" style="font-size: 30px">SNTN: 5215306-4</text>
         <g id="logo-2">
            <image style="enable-background:new;" width="787" height="440" id="image-2_2_" xlink:href="{{$path2}}/{{$selfcompanyc->logo}}"  transform="matrix(1 0 0 1 1350.4515 1829.9059)">
            </image>
         </g>
         <text transform="matrix(1 0 0 1 857.0453 2700.5581)" class="st24 st6 st25">1</text>
         <text transform="matrix(1 0 0 1 1157.0453 2700.5581)" class="st24 st6 st25">{{!empty($sql->type_of_good) ? $sql->type_of_good : ''}} {{isset($package_name) ? $package_name : ''}}</text>
         <text transform="matrix(1 0 0 1 1810.0453 2700.5581)" class="st24 st6 st25">{{(!empty($sql->weight) ? $sql->weight : '')}}</text>
...
<div class="panel-footer text-left">
   ...
   <!--<a class="btn btn-danger" id="downloadPDF" onclick="downloadDivAsPDF('printableArea1234')">Download</a>-->
   <button class="btn btn-info printableArea123" onclick="printDiv('printableArea1234')">
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/0.4.1/html2canvas.min.js"></script>
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/svg2pdf.js/1.2.0/svg2pdf.min.js"></script>
...
document.getElementById('downloadPDF').addEventListener('click', async function() {
    ...
    const svgElement = document.getElementById('printableArea1234');
    ...
    // Use svg2pdf to convert the Data URL to PDF
    await pdf.svg(url, { x: 0, y: 0 });
...
//     var printableArea = document.getElementById('printableArea1234');

//     // Use html2canvas to capture the div content as a canvas
//     html2canvas(printableArea, {
//         scale: 2, // Improve resolution by scaling the canvas
//         useCORS: true, // Allows external content to be included
//         allowTaint: true // Prevents tainted images from causing issues
//     }).then(canvas => {
//         // Convert the canvas to an image (base64 encoded)
//         var imgData = canvas.toDataURL('image/png');

//         // Load jsPDF from the global namespace
//         const { jsPDF } = window.jspdf;
...
//         // Add the image to the PDF (coordinates: x=0, y=0)
...
// <!--        html2pdf()-->
// <!--            .from(printableArea)-->
//             .save('div_content.pdf') // The name of the downloaded PDF file
// <!--            .catch(function(error) {-->
...
<script>
    function ExportToDoc_correct(filename = 'document') {
            // Get the content of the div
            var preHtml = "<html xmlns:o='urn:schemas-microsoft-com:office:office' " +
                "xmlns:w='urn:schemas-microsoft-com:office:word' " +
                "xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'>" +
                "<title>Export HTML to Word</title></head><body>";
            var postHtml = "</body></html>";
            var html = preHtml + document.getElementById("printableArea1234").innerHTML + postHtml;
    ...
    function ExportToDoc(filename = 'printableArea1234') {
    ...
    function ExportToDocOld(filename = ''){
        alert(1);
    var HtmlHead = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><title>Export HTML To Doc</title></head><body>";
...
<script>
   ...
   // select print button with class "print," then on click run callback function
    ...
    function printDiv(divId) {
            ...
            // Replace the body with the div's contents
            ...
            // Rebind the event handlers if necessary (for button or other interactive elements)
            bindPrintButton();
        }

    function bindPrintButton() {
            // Rebind the print button event after restoring the content
            const printButton = document.querySelector('.printableArea123');
