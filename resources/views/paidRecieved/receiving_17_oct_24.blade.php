                       ...
                       success:function(data)
                       {
                            ...
                            for (var i = 0; i < data.data.length; i++) {
                                if (data.data[i].r_total != '0') {
                                    ...
                                    op2 += '<td id="remains_' + data.data[i].bill_id + '"><input type="text" readonly class="form-check-input" id="bill_amount' + 
                                           data.data[i].bill_id + '" name="bill_amount[]" value="' + data.data[i].r_total + '"></td>';
                                    
                                    op2 += '<td><input id="received_' + data.data[i].bill_id + '" type="text" class="exampleCheck form-check-input" ' + 
                                           'name="received[]" value="" data-id="' + data.data[i].bill_id + '"></td>';
                                    
                                    op2 += '<td><input type="text" id="remaining_amount_' + data.data[i].bill_id + '"  class="r_amount_' +  
                                           data.data[i].bill_id + ' form-check-input" id="r_amount' + data.data[i].bill_id + 
                         ...
                         /*$("#exampleCheck").change(function() {
                          ...
                          var xyz = 0;
                           $(".exampleCheck").keyup(function() {
                              var currentInputId = $(this); // Get the ID of the current input field
                              console.log("Current Input Field ID: " + currentInputId);
                              var id = $(this).attr('data-id'); // Get the data-id attribute of the current input
                                var amount = $(this).val(); // Get the value entered in the current input
                                var bill_amount = $('#bill_amount' + id).val(); // Get the related bill amount by the data-id
                                
                                // Calculate the variation
                                var variations = bill_amount - amount;
                                var r_amount = $(this).closest('tr').find('.r_amount_' + id).val();
                            
                                // Find the closest .r_amount field within the same row and set its value to variations
                                $(this).closest('tr').find('.r_amount_' + id).val(variations);
                            
                  
                              var receivingAmountValue = $('#receiving_amount').val(); // Assuming it's an input field
                                ...
                                $(".exampleCheck").each(function() {
                                    ...
                                    if (totalReceived > receivingAmountValue) {
                                        ...
                                        $(currentInputId).val(''); // Clear the current input field
                                        $(this).closest('tr').find('.r_amount_' + id).val(r_amount);
